...
/// - Route messages by PayloadType (RPC → Mailbox, data stream → Registry)
///
/// # Design Principles
...
pub(crate) struct WebRtcGate {
    ...
    /// data stream registry for fast-path message routing
...
impl WebRtcGate {
    ...
    /// - `data_stream_registry`: data stream registry for fast-path routing
    pub fn new(
        coordinator: Arc<WebRtcCoordinator>,
        pending_requests: PendingRequestsMap,
        data_stream_registry: Arc<DataStreamRegistry>,
