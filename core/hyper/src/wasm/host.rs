...
use actr_protocol::{
    ActrError, ActrId, ActrType, DataStream, DeliveryState, MetadataEntry, PayloadType, Realm,
    RecoveryCode, RecoveryInfo, RpcEnvelope,
};
...
fn recovery_code_to_wit(code: RecoveryCode) -> wit_types::RecoveryCode {
    match code {
...
fn delivery_state_to_wit(state: DeliveryState) -> wit_types::DeliveryState {
    match state {
...
fn recovery_info_to_wit(info: &RecoveryInfo) -> wit_types::RecoveryInfo {
    wit_types::RecoveryInfo {
        peer: proto_actr_id_to_wit(&info.peer),
        session_id: info.session_id,
        code: recovery_code_to_wit(info.code),
        reason: info.reason.clone(),
        elapsed_ms: info.elapsed_ms,
        timeout_ms: info.timeout_ms,
        retry_after_ms: info.retry_after_ms,
        delivery: delivery_state_to_wit(info.delivery),
    }
}

fn wit_recovery_info_to_proto(info: wit_types::RecoveryInfo) -> RecoveryInfo {
    RecoveryInfo {
        peer: wit_actr_id_to_proto(&info.peer),
        session_id: info.session_id,
...
fn actr_error_to_wit(e: &ActrError) -> WitActrError {
    match e {
        ...
        ActrError::Recovering(info) => WitActrError::Recovering(recovery_info_to_wit(info)),
...
fn wit_actr_error_to_proto(e: WitActrError) -> ActrError {
    match e {
        ...
        WitActrError::Recovering(info) => ActrError::Recovering(wit_recovery_info_to_proto(info)),
