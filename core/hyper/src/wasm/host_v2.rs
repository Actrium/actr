...
/// Map a completed barrier (lifecycle / data-stream) outcome to a unit reply,
    ...
    match cmd {
        ...
        ActorCmd::DataStream {
            ...
            ledger
                ...
                .expect("reply ledger mutex poisoned")
                .insert(token, PendingReply::Unit(reply));
            let res = run_data_stream_region(accessor, bindings, wit_chunk, wit_sender, inv)
                .instrument(span)
                .await;
            accessor.with(|mut a| a.get().remove_invocation(token));
            if let Some(PendingReply::Unit(tx)) = ledger
                ...
                .remove(&token)
            {
                let _ = tx.send(classify_unit("on_data_stream", res));
            }
            BarrierNext::Continue
        }
        ActorCmd::Hook {
...
/// Region-internal data-stream call.
async fn run_data_stream_region(
    accessor: &Accessor<HostState>,
    bindings: &ActrWorkloadGuestV2,
    wit_chunk: WitDataStream,
