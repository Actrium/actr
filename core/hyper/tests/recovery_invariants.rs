...
//! `ConnectionSupervisor` engine. It complements, rather than replaces, the
//! deep synchronous unit coverage in `src/lifecycle/connection_supervisor_tests.rs`
...
//! |---|---|---|
//! | 1 | Stale/duplicate snapshot inert; new epoch resets sequence | here: `inv1_stale_and_duplicate_snapshots_cannot_advance_policy_but_a_new_epoch_does`; unit: `connection_supervisor_tests::inv1_duplicate_and_stale_snapshots_cannot_move_path_state`; reducer: `translate_tests::{snapshot_stale_epoch_or_sequence_is_discarded, snapshot_structural_duplicate_is_discarded}` |
//! | 2 | Fast rollback: no disconnect, supersedes/cancels pending or running disconnect, derives restoration | here: `inv2_online_rollback_before_grace_performs_no_disconnect`; unit: `connection_supervisor_tests::{inv2_online_before_grace_rolls_back_without_disconnect, inv2_online_cancels_a_running_confirmed_disconnect_and_derives_restoration}`; reducer: `translate_tests::snapshot_online_supersedes_pending_disconnect` |
//! | 3 | `OfflineCandidate` permits existing traffic, no new negotiation, no teardown barrier | here: `inv3_offline_candidate_permits_existing_session_without_immediate_disconnect`; unit: `connection_supervisor_tests::inv3_offline_candidate_arms_grace_and_permits_existing_traffic` (asserts `SendProjection::ExistingOnly` directly, not observable through the public API) |
//! | 4 | Cleanup cannot acknowledge a later recovery fact | unit: `connection_supervisor_tests::{inv4_cleanup_supersedes_pending_recovery_by_revision, inv4_cleanup_completion_cannot_acknowledge_a_later_recovery_fact}`; integration: `network_event_debounce::test_l1_cleanup_is_a_batch_barrier_for_later_recovery_facts` |
...
//! | 25 | No effect classifies its own failure; verdict from the per-kind classification table alone | unit: `connection_supervisor_tests::inv25_classification_table_alone_determines_the_verdict`; structurally enforced by `EffectOutcome::Failed` carrying only a typed `diagnosis` field (`recovery_policy::diagnosis`), never a verdict |
...
//! | 27 | Every non-`Idle` execution gets exactly one accepted terminal completion; only the supervisor mutates it | unit: `connection_supervisor_tests::inv27_cleanup_preempts_a_running_probe`; "only the supervisor mutates it" is a structural property (`ConnectionSupervisor::view().execution` is private, mutated only by `apply_execution` in the same module) |
//! | 28 | Obligations end only through enumerated extinguishment paths; fact reversal extinguishes only its own domain | unit: `connection_supervisor_tests::inv28_fact_reversal_extinguishes_only_its_own_domain` |
//! | 29 | `DisconnectPending` implies `NetworkPath == Offline`; fall-through logged, never silent | unit: `connection_supervisor_tests::inv29_offline_grace_expiry_commits_and_derives_confirmed_disconnect`; the logging half is a code-review property of the translation table's `_ =>` arms (each logs `tracing::error!`), not independently unit-tested |
//! | 30 | Teardown/shutdown terminate within their overall deadline; only unbounded state is `InvariantViolation` park outside `Terminating` | unit: `connection_supervisor_tests::inv30_app_terminating_enters_terminating_and_blocks_sends`; reducer: `translate_tests::{teardown_deadline_abandons_backing_off_obligation, teardown_deadline_ignored_while_effect_running, shutdown_deadline_aborts_when_terminating_and_matching}`, `classification::teardown_invariant_violation_parks_or_abandons_by_mode` |
