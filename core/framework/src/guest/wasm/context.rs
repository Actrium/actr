...
//!
//! Replaces the pre-Phase-1 `actr_host_invoke` / ptr-len ABI bridge with
//! direct calls to the `actr:workload/host` imports generated by
//! [`super::generated`]. Every method is an `async fn` on the generated
//! surface — even imports declared sync in WIT — because
//! `wit_bindgen::generate!` emits a uniform async API when `async: true`
//! is set (see the Phase 0.5 spike report for why). No cooperative
