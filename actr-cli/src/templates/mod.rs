...
"#
    ...
    // Build with protoc-gen-actorframework plugin if available
    let plugin_path = std::env::current_dir()?
        .parent()
        .unwrap()
        .join("target/debug/protoc-gen-actorframework");
    ...
    if plugin_path.exists() {
        config = config
            .protoc_arg(format!("--plugin=protoc-gen-actorframework={}", plugin_path.display()))
            .protoc_arg("--actorframework_out=.");
        println!("Using protoc-gen-actorframework plugin");
    } else {
        println!("Warning: protoc-gen-actorframework plugin not found, using standard tonic generation only");
    }
