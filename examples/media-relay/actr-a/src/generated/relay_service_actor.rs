...
#[doc = r" }"]
#[doc = r" ```"]
pub trait RelayServiceHandler: Send + Sync + 'static {
...
pub struct RelayServiceWorkload<T: RelayServiceHandler>(pub T);
impl<T: RelayServiceHandler> RelayServiceWorkload<T> {
...
#[doc = r" - 编译器完全内联"]
pub struct RelayServiceDispatcher<T: RelayServiceHandler>(std::marker::PhantomData<T>);
#[async_trait]
impl<T: RelayServiceHandler> MessageDispatcher for RelayServiceDispatcher<T> {
    type Workload = RelayServiceWorkload<T>;
    async fn dispatch<C: Context>(
        ...
        match envelope.route_key.as_str() {
            "media_relay.RelayService.RelayFrame" => {
                ...
                let req = RelayFrameRequest::decode(&**payload).map_err(|e| {
                    actr_protocol::ProtocolError::Actr(actr_protocol::ActrError::DecodeFailure {
                        message: format!(
                            "Failed to decode {}: {}",
                            stringify!(RelayFrameRequest),
                            e
                        ),
                    })
                })?;
                let resp = workload.0.relay_frame(req, ctx).await?;
...
impl<T: RelayServiceHandler> Workload for RelayServiceWorkload<T> {
    type Dispatcher = RelayServiceDispatcher<T>;
