...
cleanup() {
    ...
    if [ -n "$SERVER_PID" ]; then
        echo "Stopping wasm-echo-server (PID: $SERVER_PID)"
        kill $SERVER_PID 2>/dev/null || true
    fi

    if [ -n "$CLIENT_PID" ]; then
        echo "Stopping wasm-echo-client (PID: $CLIENT_PID)"
...
# Build guest crate for wasm32
cd "$GUEST_DIR"
cargo build --release --target wasm32-unknown-unknown 2>&1 | tail -5

RAW_WASM="$GUEST_DIR/target/wasm32-unknown-unknown/release/wasm_echo_guest.wasm"
if [ ! -f "$RAW_WASM" ]; then
    echo -e "${RED}❌ WASM build failed: $RAW_WASM not found${NC}"
    exit 1
fi

echo -e "${GREEN}✅ WASM guest built: $(du -h "$RAW_WASM" | cut -f1) ${NC}"

# Apply wasm-opt --asyncify transform
echo ""
echo -e "${BLUE}🔧 Applying wasm-opt --asyncify transform...${NC}"
echo "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
...
if command -v wasm-opt > /dev/null 2>&1; then
    ...
    for candidate in \
        "$HOME/.cache/.wasm-pack/wasm-opt-*/bin/wasm-opt" \
        "$HOME/.cargo/bin/wasm-opt" \
        "/usr/local/bin/wasm-opt" \
        "/opt/homebrew/bin/wasm-opt"; do
        # Use glob expansion
...
if [ -z "$WASM_OPT" ]; then
    echo -e "${YELLOW}⚠️  wasm-opt not found. Installing via cargo install wasm-opt...${NC}"
...
BUILT_DIR="$GUEST_DIR/built"
mkdir -p "$BUILT_DIR"

"$WASM_OPT" --asyncify -O "$RAW_WASM" -o "$BUILT_DIR/wasm_echo_guest.wasm"

if [ ! -f "$BUILT_DIR/wasm_echo_guest.wasm" ]; then
    echo -e "${RED}❌ wasm-opt --asyncify failed${NC}"
    exit 1
fi
...
while [ $COUNTER -lt $MAX_WAIT ]; do
    if ! kill -0 $SERVER_PID 2>/dev/null; then
        echo -e "${RED}❌ WASM server failed to start${NC}"
        cat "$LOG_DIR/wasm-echo-server.log"
        exit 1
    fi

    if grep -q "WASM Echo Server fully started\|ActrNode started" "$LOG_DIR/wasm-echo-server.log" 2>/dev/null; then
        echo -e "${GREEN}✅ WASM server is running and registered${NC}"
        break
    fi
...
(
    ...
    sleep 2
    echo "quit"
) | RUST_LOG="${RUST_LOG:-info}" cargo run --bin wasm-echo-client > "$LOG_DIR/wasm-echo-client.log" 2>&1 &
CLIENT_PID=$!
...
if grep -q "\[Received reply\].*Echo: $TEST_INPUT" "$LOG_DIR/wasm-echo-client.log"; then
    ...
    echo ""
    echo "📖 View full logs:"
    echo "   cat $LOG_DIR/wasm-echo-client.log  # Client logs"
    echo "   cat $LOG_DIR/wasm-echo-server.log  # WASM server logs"
    echo "   tail -f $LOG_DIR/actrix.log        # Actrix logs"
    echo ""
    exit 0
else
    echo -e "${RED}❌ Test FAILED: Expected WASM echo server response not found${NC}"
    echo -e "${RED}   Looking for: [Received reply] Echo: $TEST_INPUT${NC}"
    echo ""
    echo "Client output:"
    cat "$LOG_DIR/wasm-echo-client.log"
    echo ""
    echo "Server output (last 30 lines):"
    tail -30 "$LOG_DIR/wasm-echo-server.log"
    echo ""
