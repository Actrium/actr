//! Shell Workload — empty placeholder for WASM executor mode
//!
//! When a WASM executor adapter is attached via `ActrNode::with_wasm_instance()`,
//! incoming messages are dispatched through the WASM instance instead of the native
...
/// No-op workload used as placeholder when WASM executor handles all dispatch
#[derive(Default)]
...
impl MessageDispatcher for ShellDispatcher {
    ...
    async fn dispatch<C: Context>(
        ...
        // This should never be reached when WASM executor is configured
        Err(ActrError::UnknownRoute(format!(
            "ShellWorkload: dispatch should not be called in WASM mode (route_key={})",
            envelope.route_key
        )))
    }
}
