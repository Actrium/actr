...
在本框架中，一个设计良好的 `Actor` 通常具有良好的可测试性。这得益于框架的核心设计：`Actor` 的业务逻辑（你实现的 `trait`）与框架的运行时（`ActorSystem`）通过 `Context` 和 `Adapter` 实现了彻底的解耦。
...
async fn test_register_new_user_schedules_welcome_email() {
    ...
    // 调用 trait 方法，传入 mock context
    let response = actor.register_user(request, Arc::new(mock_context)).await;
...
graph TD
    ...
    subgraph "被测单元 (Unit Under Test)"
        Actor["<b>Actor 实例</b><br/><i>(UserActor)</i>"]
    end
    ...
    TestFn -- "3. 调用 trait 方法<br/>(注入 MockContext)" --> Actor
...
为了在不依赖真实网络和信令服务器的情况下进行测试，我们创建了一个 `InMemorySignaling` 适配器。它使用 `mpsc::channel` 来模拟客户端与 `ActorSystem` 之间的信令交换。
...
2.  **编写集成测试**
    ...
    async fn test_echo_service_end_to_end() {
        ...
        // c. 创建一个模拟客户端，它也使用内存信令
        let mut client = MockClient::new(Box::new(client_signaling));
        ...
        // 客户端通过其 WebRTC 数据通道发送一个真实的请求
        let response = client.send_rpc(
...
*   **集成测试**作为一层安全网，确保你的 `Actor` 在真实的（尽管是模拟的网络）`ActorSystem` 环境中能够正确运行。为每个服务的关键路径编写一到两个集成测试用例，是保证系统可靠性的良好实践。
