# 专题解析：信令机制 (Signaling)

WebRTC 技术本身只负责建立点对点 (Peer-to-Peer) 的直接连接，但它无法解决一个初始问题：“我应该如何发现并连接到另一个对等端？” 这个“发现与协商”的过程，就是**信令 (Signaling)**。

本框架不提供内置的信令服务器，而是定义了一套清晰的信令协议。开发者需要提供一个实现了该协议的信令服务器，以“引导”框架完成连接。

## 1. 框架对信令服务器的核心期望

框架期望信令服务器扮演一个“智能路由与授权中心”的角色。其核心职责是：

1.  **管理连接**: 接受并管理来自各个 Actor (客户端) 的 WebSocket 连接。
2.  **身份与路由**: 维护一个 `ActorId` 到 WebSocket 连接的映射表，以便能根据 `ActorId` 将消息路由到正确的客户端。
3.  **ACL 感知的 Presence 管理**: 这是信令服务器最关键的职责之一。在广播一个新 Peer 上线时，必须为每个已在线的接收方，根据 ACL 规则检查其是否有权限“发现”这个新 Peer。**只将允许被发现的 Peer 信息通知给对方**。
4.  **消息转发**: 可靠地转发遵循框架[信令层契约](./1.2-Framework-Internal-Protocols.zh.md#2-信令层契约-signalingproto)的消息。

> [!IMPORTANT]
> 框架的信令协议定义在 `signaling.proto` 中。任何自定义的信令服务器都必须能正确地处理该文件中定义的 `SignalingMessage`。
> 详情请参阅《[1.2 框架内部协议](./1.2-Framework-Internal-Protocols.zh.md)》。

## 2. 核心信令交互流程 (含 ACL)

“发现即授权”是本框架信令交互的核心安全原则。客户端收到的 `NewActor` 消息，本身就意味着它已被授权可以与该 Peer 发起连接。

```mermaid
sequenceDiagram
    participant Alice
    participant SignalingServer as 信令服务器
    participant Bob

    Alice->>+SignalingServer: 1. 连接并认证
    SignalingServer-->>-Alice: 认证成功

    Bob->>+SignalingServer: 2. 连接并认证 (携带类型信息)
    SignalingServer-->>-Bob: 认证成功

    Note over SignalingServer: Bob 上线，开始处理 Presence 通知

    par ACL Check for Alice
        SignalingServer->>SignalingServer: 检查: can_discover(Alice, Bob)?
        alt ACL 检查通过
            SignalingServer-->>Alice: 3. NewActor (Bob)
        else ACL 检查失败
            SignalingServer-->>Alice: (不发送任何通知)
        end
    and ACL Check for Bob
        SignalingServer->>SignalingServer: 检查: can_discover(Bob, Alice)?
        alt ACL 检查通过
            SignalingServer-->>Bob: 3. NewActor (Alice)
        else ACL 检查失败
            SignalingServer-->>Bob: (不发送任何通知)
        end
    end

    Note over Alice, Bob: 假设双方 ACL 检查通过，互相收到了通知

    Alice->>+SignalingServer: 4. WebRTCSignal (target=Bob, data=Offer)
    Note right of Alice: 我知道Bob的存在，所以我被授权连接
    SignalingServer->>+Bob: WebRTCSignal (source=Alice, data=Offer)
    Bob-->>-SignalingServer: (处理 Offer...)

    Note over Alice, Bob: 后续 Answer/ICE 流程同理...
```

**流程解析**:
1.  **连接与认证**: Alice 和 Bob 各自连接到信令服务器。
2.  **ACL 感知发现**: 当 Bob 上线时，信令服务器**不会**立即广播。而是：
    *   检查 Alice 是否有权限发现 Bob。如果有，向 Alice 发送 `NewActor` 通知。
    *   检查 Bob 是否有权限发现 Alice。如果有，向 Bob 发送 `NewActor` 通知。
3.  **发起连接**: Alice 因为收到了关于 Bob 的 `NewActor` 通知，所以它知道自己有权连接 Bob。在需要时，她的 `ActorSystem` 会创建一个 `Offer` 并发送，正式开始连接过程。
4.  **隐式授权转发**: 信令服务器收到 `WebRTCSignal` 后，（因为发现阶段已经保证了可连接性，且 Bob 的 `ActorSystem` 会再做核查）**原则上可以不再重复检查 ACL**，直接将其转发给目标。这简化了信令服务器的逻辑。
5.  后续的 `Answer` 和 `ICE` 交换流程不变。

## 3. 实现自定义信令服务器的简要指南

实现一个满足框架要求的信令服务器并不复杂。以下是一个基于 Node.js 和 `ws` 库的最小化实现思路：

```javascript
import { WebSocketServer } from 'ws';

// 用于存储 ActorId -> WebSocket 连接的映射
const clients = new Map();

const wss = new WebSocketServer({ port: 8080 });

wss.on('connection', ws => {
  let actorId = null; // 在认证成功后设置

  ws.on('message', message => {
    // 1. 解析 Protobuf 消息
    // const signalingMessage = SignalingMessage.decode(message);
    
    // 2. 处理认证逻辑 (假设有一个 AuthRequest 消息)
    // if (signalingMessage.authRequest) {
    //   actorId = performAuth(signalingMessage.authRequest);
    //   clients.set(actorId, ws);
    //   // 广播 NewActor... (需要注意 ACL 规则)
    // }

    // 3. 处理 WebRTCSignal 转发逻辑
    // if (signalingMessage.webrtcSignal) {
    //   const targetWs = clients.get(signalingMessage.webrtcSignal.target);
    //   if (targetWs) {
    //     // 直接将原始 Protobuf 数据转发给目标客户端
    //     targetWs.send(message);
    //   } else {
    //     // 发送错误：Actor Unreachable
    //   }
    // }
  });

  ws.on('close', () => {
    if (actorId) {
      clients.delete(actorId);
      // 广播 ActorLeft...
    }
  });
});
```

**核心要点**:
*   使用一个 `Map` 来维护 `ActorId` 和 `WebSocket` 实例的在线状态。
*   服务器的主要工作是解析顶层的 `SignalingMessage`，检查其类型。
*   对于 `WebRTCSignal` 类型的消息，服务器只需扮演“邮差”的角色，根据其 `target` 字段找到对应的 `WebSocket` 连接，然后将**未经修改的原始消息**转发过去即可。框架的客户端 `ActorSystem` 会处理具体的 `Offer/Answer/ICE` 内容。
*   在生产环境中，你还需要考虑水平扩展、认证安全、错误处理以及 ACL 规则的实施等更多细节。
