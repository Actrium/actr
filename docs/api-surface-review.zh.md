...
4. **wire 语义仍压在应用面上**：流 API 已统一为 `send_data_chunk(chunk: DataChunk)`，但调用方仍要直接选择 `PayloadType::StreamReliable` / `StreamLatencyFirst` 这类 wire lane。
...
12. 流 API 的 lane 选项下沉为更窄的应用层配置，避免应用代码直接依赖 `PayloadType` 的 wire 枚举。
