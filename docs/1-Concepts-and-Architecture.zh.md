...
*   **2.3. 架构基石：双路径处理模型**
    ...
    *   **状态路径 (The State Path)**：此路径用于处理需要与 Actor 核心状态交互的、必须保证顺序性和原子性的消息。例如，服务的建立与销毁、元数据的交换、状态变更请求等。所有送往状态路径的消息，都会被首先提交到 Actor 的 **Mailbox** 中。

        Mailbox 是一个逻辑概念，代表了 Actor 的**持久化、有序的事务入口**。就实现而言，它是一个基于 **WAL (Write-Ahead Log)** 为基础的高性能队列系统。消息在写入持久化日志后，才会被放入**两条不同优先级的内存队列**中，等待调度器处理：
...
*   **2.5. 两种运行模式：库模式与自动运行器**

    为了将选择权交给用户，框架通过配套的命令行工具 `actr-cli` 提供了两种截然不同的运行模式，开发者可以根据项目的复杂度和需求自由选择，就像 `cargo` 可以创建 `lib` 或 `bin` 项目一样。

            *   **库模式 (Library Mode)**: 在此模式下，开发者拥有对进程的完全控制权。你需要亲手编写 `main.rs` 入口文件来组装应用。此模式的核心应用场景有两个：一是将 `ActorSystem` **作为组件嵌入到一个已有的复杂应用中**；二是适用于需要对进程进行精细化控制（如自定义配置、依赖注入等）的新项目。

    *   **自动运行器模式 (Auto-Runner Mode)**: 此模式提供了“开箱即用”的便利性。开发者只需专注于实现 `LocalActor` 的业务逻辑（在一个库文件中），而无需编写 `main.rs`。`actr-cli` 工具在构建时会自动检测到入口文件的缺失，并根据 `actr.toml` 中的配置，为您生成一个标准的启动程序。

    这种双模式设计，兼顾了框架的灵活性与易用性。
