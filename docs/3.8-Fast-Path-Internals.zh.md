...
sequenceDiagram
    ...
    Client->>ActorSystem: 1. OpenStreamRequest (RPC)
    note right of Client: (例如，请求上传一个文件)

    ActorSystem->>StatePath: 2. 调用 on_open_stream 方法
    ...
    StatePath-->>ActorSystem: 4. OpenStreamResponse
    ActorSystem-->>Client: OpenStreamResponse

    loop 数据传输
        Client->>ActorSystem: 5. Stream Data Packet (走 Fast Path)
...
**核心思想**:
1.  **注册时**: `State Path` 在注册回调时，不传递 `Actor` 自身，而是创建一个包含了 `ActorId` 和 `Context` 的轻量级**并发句柄 (`ConcurrentHandle`)**。
