...
extend google.protobuf.ServiceOptions {
  bool force_generate_client_helpers = 50004;
}
...
```protobuf
// ✅ 每个 rpc 都有自己专属的“信封”
message UpdateUserRequest { UserProfile profile = 1; }
message UpdateUserResponse { bool success = 1; }
rpc UpdateUser(UpdateUserRequest) returns (UpdateUserResponse);

message GetUserRequest { string user_id = 1; }
message GetUserResponse { UserProfile profile = 1; }
...
*   **做法**: 使用 `import` 引入其 `.proto` 文件，然后在代码中通过 `ctx` 上由框架自动生成的、类型安全的辅助方法来调用。
    ```rust
    // 调用在 other_service.proto 中定义的服务
    let response = ctx.call_get_user_data(GetUserRequest { ... }).await?;
    ```
