...
impl LanguageGenerator for SwiftGenerator {
    async fn generate_infrastructure(&self, context: &GenContext) -> Result<Vec<PathBuf>> {
        ...
        let local_actrframework_plugin = self.ensure_required_tools()?;
    ...
    async fn generate_scaffold(&self, context: &GenContext) -> Result<Vec<PathBuf>> {
        ...
        let user_file_path = layout
            .as_ref()
            .map(|l| l.mutable_scaffold.clone())
            .unwrap_or_else(|| {
                context
                    .output
                    .parent()
        ...
        // Check if ActrService.swift exists and should be overwritten. Skip it
        // (rather than hard-returning) so the LifecycleAdapter scaffold below
        // is still generated — upgrading an existing project picks up the new
        // adapter without forcing --overwrite-user-code.
        let write_service = if !user_file_path.exists() {
            true
        } else {
        ...
        let write_adapter = if !adapter_file_path.exists() {
            ...
            let is_scaffold =
                self.should_overwrite_scaffold(&adapter_file_path, &adapter_content)?;
            if is_scaffold {
                info!("🔄 Overwriting scaffold file: {:?}", adapter_file_path);
                true
            } else if !context.overwrite_user_code {
                info!(
                    "⏭️  Skipping existing LifecycleAdapter scaffold: {:?}",
                    adapter_file_path
                );
        ...
        if write_adapter {
            std::fs::write(&adapter_file_path, &adapter_content).map_err(|e| {
                ActrCliError::config_error(format!(
                    "Failed to write LifecycleAdapter scaffold: {e}"
                ))
            })?;
            info!(
                "📄 Generated LifecycleAdapter scaffold: {:?}",
                adapter_file_path
            );
            scaffold_files.push(adapter_file_path);
        }
...
impl SwiftGenerator {
    fn ensure_required_tools(&self) -> Result<Option<PathBuf>> {
    ...
    /// Check that the installed Swift plugin matches the actr release version.
    fn check_plugin_version(&self) -> Result<()> {
        let cli_version = env!("CARGO_PKG_VERSION");
        let Some(plugin_version) = self.get_plugin_version()? else {
            return Err(ActrCliError::command_error(
                "Could not determine protoc-gen-actrframework-swift version".to_string(),
            ));
        };
        ...
        Err(ActrCliError::command_error(format!(
            "protoc-gen-actrframework-swift version {} does not match actr version {}. Install the plugin asset from the matching ACTR release.",
            plugin_version, cli_version
        )))
    }
...
impl SwiftGenerator {
    ...
    ///
    /// The handler type name follows `protoc-gen-actrframework-swift`'s
    /// convention: `<ServiceName>Handler` (impl: `<ServiceName>HandlerImpl`).
    /// It is derived from the workload name (`<ServiceName>Workload`) so the
    /// generated adapter matches the generated actor's types.
    fn generate_lifecycle_adapter_content(
        &self,
        service_name: &str,
        workload_name: &str,
    ) -> Result<String> {
        let handler_name = workload_name
            .strip_suffix("Workload")
            .map(|stem| format!("{stem}Handler"))
            .unwrap_or_else(|| format!("{service_name}Handler"));

        #[derive(Serialize)]
        struct LifecycleAdapterContext {
            #[serde(rename = "SERVICE_NAME")]
            service_name: String,
            #[serde(rename = "WORKLOAD_NAME")]
            workload_name: String,
