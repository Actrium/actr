/* Actor-RTC Service Worker entry — Option U / wasm-bindgen guest variant.
 *
 * Parallel entry to `actor.sw.js` (Component Model + jco path). This variant
 * loads a **wasm-bindgen** guest produced by `wasm-pack --target no-modules`
 * instead of a Component Model + jco bundle. It exists so Option U (see
 * docs/option-u-wit-compile-web.zh.md) can run end-to-end without disturbing
 * the CM pipeline that other examples (data-stream-peer-concurrent) still
 * rely on.
 *
 * Selected by `cli/commands/run.rs` when `ACTR_WEB_GUEST_MODE=wbg` is set.
 *
 * Required `runtimeConfig` fields (unchanged from actor.sw.js):
 ...
 *   be served alongside the .actr; CLI mounts them when the companion
 *   directory exists (mirrors the `.jco/` directory convention).
 */
...
async function loadWithWbgBridge(packageUrl, runtimeWasmUrl) {
    emitSwLog('info', 'wbg_bridge_start', { packageUrl, runtimeWasmUrl });
    ...
    (0, eval)(patchedText);
    emitSwLog('info', 'wbg_bridge_runtime_js_loaded', jsText.length);

    await wasm_bindgen({ module_or_path: runtimeWasmUrl });
    wasm_bindgen.init_global();
    emitSwLog('info', 'wbg_bridge_runtime_ready', null);
    ...
    emitSwLog('info', 'wbg_bridge_actr_size', buffer.byteLength);
    ...
    try {
        ...
        emitSwLog('info', 'wbg_bridge_verify_ok', null);
    } catch (verifyError) {
        emitSwLog('error', 'wbg_bridge_verify_failed', String(verifyError));
        throw verifyError;
    }
    ...
    emitSwLog('info', 'wbg_bridge_guest_js_url', wbgJsUrl);
    ...
    emitSwLog('info', 'wbg_bridge_guest_wasm_url', wbgWasmUrl);
    await self.actrGuestBindgen({ module_or_path: wbgWasmUrl });
    emitSwLog('info', 'wbg_bridge_guest_instantiated', Object.keys(self.actrGuestBindgen));
    ...
    emitSwLog('info', 'wbg_bridge_ready', 'WBG workload registered');
}
...
async function ensureWasmReady() {
    ...
    try {
        ...
        await loadWithWbgBridge(packageUrl, runtimeWasmUrl);
