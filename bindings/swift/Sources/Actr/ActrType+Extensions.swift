...
public extension ActrType {
    ...
    static func fromStringRepr(_ stringRepr: String) throws -> ActrType {
        guard let plusIndex = stringRepr.firstIndex(of: "+") else {
            throw ActrError.ConfigError(msg: "Invalid ActrType format: '\(stringRepr)'. Expected format: manufacturer+name:version (e.g., acme+EchoService:1.0.0)")
        }
        ...
        guard !version.isEmpty else {
            throw ActrError.ConfigError(msg: "Invalid version: version cannot be empty")
        }
