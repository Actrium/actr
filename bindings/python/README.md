...
The Python binding is now `package-first`. Source-defined local workloads were removed. The current Python API boots an `ActrNode` wrapper from `manifest.toml`, auto-loads the sibling `actr.toml`, then uses discovery plus explicit remote calls.
...
- `ActrNode.from_toml(path)` boots an `ActrNode` wrapper from `manifest.toml`
...
shape for the current Python binding surface. If you need fine-grained
control (custom `TrustProvider`, pre-built `Hyper`, workload hosting,
etc.), drop down to the `actr_hyper::{Hyper, Node}` API in native Rust.
