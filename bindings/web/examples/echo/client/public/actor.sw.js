...
// We intercept these calls and broadcast structured events to the main
// page so the UI log panel can display real-time runtime information.
(function () {
  ...
  function extractMessage(args) {
    ...
    // Strip CSS-only arguments to keep the UI clean.
  ...
  console.warn = function (...args) {
    ...
    const msg = extractMessage(args);
    if (msg.includes('[SW]') || msg.includes('Echo') || msg.length > 0) {
  ...
  console.log = function (...args) {
    ...
    const msg = extractMessage(args);
    if (msg.includes('[SW]') || msg.includes('Echo') || msg.includes('[WebRTC]')) {
      broadcast({ type: 'sw_log', level: 'info', message: msg, ts: Date.now() });
    }
  };
})();
