...
**实际原因**：主 worktree 的 `target/debug/actr` binary 是 14:12 的旧版本；Phase 6b 和 TD-004 α' cherry-pick 之后**忘记重新 `cargo build -p actr-cli`**。那个 stale binary 用的是几代之前的 `cli/assets/web-runtime/actor-wbg.sw.js`，与最新的 `actr-web-abi` / WebWorkloadAdapter 不匹配，于是 dispatchFn 报 `Reflect.get called on non-object`。
...
## TD-006：MultiTab 6-2/6-3/6-4/6-5 在 TD-004 α' + Phase 6c 之后仍 FAIL
