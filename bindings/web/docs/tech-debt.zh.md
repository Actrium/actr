...
收录浏览器侧曾经登记过的技术债。当前截至 2026-04-25，TD-001 到 TD-006 都已关闭；本文主要作为历史索引和事故复盘入口，不应把旧“暂缓理由 / 修复选项”理解为仍待处理的当前债务。
...
当时补一个 sync 步骤本身工作量很小，但决定"谁调它"要看整体 web 构建流水线的责任划分；历史讨论里曾提到一个旧的 web 构建入口占位，但它不应再被描述为当前顶层入口。当前文档化路径应以 `crates/sw-host/build.sh` 和 `scripts/sync-cli-assets.sh --build` 为准。T18 主线更紧，先登记。
...
## TD-003（**已解决**）：guest dispatch context 单例，并发场景互相覆盖

**状态**：已解决（2026-04-25）。最终采用 γ 路线：host imports 线程改为携带 `request_id`，sw-host 使用 `DISPATCH_CTXS: HashMap<RequestId, Ctx>` 查找每次 dispatch 的上下文；当前 `actor.sw.js` 调用 `register_guest_workload`，不再使用旧 `actor-wbg.sw.js` 作为当前入口。
...
| β | 在旧 `actor-wbg.sw.js` 的 JS 桥层 serialize（一个 dispatch 结束再派下一个） | 一处 | 无（排队） |
| γ | host imports 签名加 request_id 参数；sw-host 用 HashMap\<RequestId, Ctx\> 查找 | WIT + codegen + sw-host | 真并发 |
| δ | 每次 dispatch 在 JS 层构造独立的 host imports 闭包捕获 ctx，替换全局 `self.actrHost*` | 旧 `actor-wbg.sw.js` 重写；但 wasm 侧 `__host_*` import 查表是 binding-time，不是每次调用 | 不可行或很 hack |
...
## TD-004（**已解决**）：同源 SW 下第二个 client 注册破坏第一个 client 的 RPC 响应

**状态**：已解决（2026-04-25）。直接根因（credential namespace 按 actr-type 共享）已通过 per-client namespace 修复；后续残留的 MultiTab RPC 挂起已收敛到 [TD-006](./td-006-multi-client-rpc-analysis.zh.md)，并已在组合修复后关闭。
...
3. ~~旧 `actor-wbg.sw.js` port 管理~~：MessagePort 表错配
...
| 3 旧 `actor-wbg.sw.js` port 管理 | **证伪**：`clientPorts: Map<clientId, port>` 每 tab 独立 |
...
- **β'**：若是 AIS 会话冲突，旧 `actor-wbg.sw.js` 给每 client 独立 session
- **γ'**：接受"1 SW 1 client"，MultiTab 6-2/6-3/6-4 正式 skip（与 `set_workload` 单例的 6-5 同类处理）

### TD-004 α' 实施后的实际状态

**已应用**（commits `eb034d94`、`b658d4f0`、`1ac5afd4`）：
- `cred_kv_namespace` 改为 `actr_credentials_{actr_type}_{client_id}`
- mock-actrix `handle_connection` 加 rebind WARN

**实测效果**：
- BasicFunction 6/6 ✓（单 client 无回归）
- MultiTab 6-1 Two Client Tabs ✓
- mock-actrix log 确认两 client 现在拿**独立** serial（4 和 5）；`WS actor rebound` WARN 未 fire（表明原 bug 路径不复现）

**历史后续**：TD-004 α' 后 MultiTab 6-2/6-3 仍 FAIL，当时说明 TD-004 不是唯一 blocker。该残留问题后来进入 TD-006，并已由组合修复关闭。

---

## TD-005（**已解决 / 误报**）：WebWorkloadAdapter + protoc-gen 看似有 `Reflect.get called on non-object`

**状态**：误报 / 已解决（2026-04-24）
**实际原因**：主 worktree 的 `target/debug/actr` binary 是 14:12 的旧版本；Phase 6b 和 TD-004 α' cherry-pick 之后**忘记重新 `cargo build -p actr-cli`**。那个 stale binary 用的是几代之前的 `cli/assets/web-runtime/actor-wbg.sw.js`（Phase 8 前旧名），与最新的 `actr-web-abi` / WebWorkloadAdapter 不匹配，于是 dispatchFn 报 `Reflect.get called on non-object`。

**验证**：重新 `cargo build -p actr-cli` 后，Phase 6c 的 3 commits 直接 cherry-pick 到 main，BasicFunction 6/6 PASS。Phase 6c 代码本身无 bug，`WebWorkloadAdapter` / `EchoServiceDispatcher` 交互正常。

**教训**：这条再次暴露 **[TD-002](#td-002)** 的影响面——改完 sw-host Rust **或**改完任何会影响 `cli/assets/web-runtime/` 内容的代码后，必须重 build `actr-cli` binary，否则 `include_bytes!` 嵌入的还是老版本。建议：
- a) Phase 5 里把"rebuild actr binary"加入 CI 的必跑步骤
- b) 或者实装 [TD-002](#td-002) §修复选项的 B（`sync-cli-assets.sh`）

Phase 6c 的 3 commits（`87b3401d` / `a4ec581b` / `cff7f71a`）已合入 main，echo 的 `*-guest-wbg` 已删除，`*-guest` 单 crate 跨 target。

---

## TD-006（**已解决**）：MultiTab 6-2/6-3/6-4/6-5 在 TD-004 α' + Phase 6c 之后仍 FAIL

**状态**：已解决（2026-04-25，commit `301c58d6`）。详见 [TD-006 深度分析](./td-006-multi-client-rpc-analysis.zh.md)，最终不是单点修复，而是 mock-actrix relay 精确投递、stale peer cleanup、DOM/SW client lifecycle、MessagePort 串行化、lane-0 重绑和 pending RPC reject 等组合修复。验证结果：`SUITES='BasicFunction MultiTab' bash start-mock.sh` 12/12 全过，`SUITES=BasicFunction` 6/6 全过。

### 历史描述（保留作为上下文）

**发现时间**：2026-04-24
**发现路径**：Phase 6c + TD-004 α' 合并后跑 MultiTab 全套件

### 现象

Phase 6 主体完成后 MultiTab 最终结果 **8/12 PASS**：

| 测试 | 结果 | 备注 |
|------|------|------|
| BasicFunction 1-1 ~ 1-6 | 6/6 ✓ | |
| MultiTab 6-1 Two Client Tabs | ✓ | |
| **MultiTab 6-2 Concurrent Multi-Client** | ✗ | "Echo not working within 60000ms" |
| **MultiTab 6-3 Close One Client** | ✗ | 同 |
| **MultiTab 6-4 Refresh One Client** | ✗ | 同 |
| **MultiTab 6-5 Multiple Server Instances** | ✗ | 同（此外受 register_workload 单例限制） |
| MultiTab 6-6 Shared SW Isolation | ✓ | Phase 6 之前是 FAIL；6-6 现在过了 |

### 定位

- TD-004 α' 确认有效：mock-actrix log 显示每个 client 独立 serial，`WS actor rebound` WARN 不再 fire
- γ 并发隔离架构（HashMap<RequestId, Ctx>）已落地
- **但 2 clients 并发 / 生命周期场景下首个 RPC 仍挂 60s**

### 嫌疑方向

和 TD-004 诊断报告 "次要发现" 一致：
> "client1 warmup 在 client2 还没出现前就已经失败（30s 无 📥）"

可能的根因（尚未深挖）：
1. **WebRTC 多 client 建联路径**：2 个 client 同时尝试和 server 建 DataChannel，可能有 signaling race / ICE candidate 冲突
2. **sw-host 的另一处隐含单例**：除了 GUEST_CTX（γ 已修）、cred_kv（TD-004 已修），可能还有 WebRTC coordinator / signaling client 等组件假设 single-client
3. **`register_workload` 单例副作用**：6-5 的"多 server"和 6-2/6-3/6-4 可能共享"第二 workload 注册 panic"这类副作用（需验证）

### 历史暂缓理由

- Phase 6 主体已达成（γ-unified + entry! 宏 + 跨 target echo + 8/12 e2e）
- 这是**第三层** bug（TD-003 修 ctx 单例、TD-004 修 cred namespace、TD-006 再深一层）
- 需要独立 diagnostic spike 锁定具体 component，不适合一次塞进 Phase 6
...
- **a''**：深挖 sw-host 找到下一个单例 / 共享状态并加 client_id 隔离
- **b''**：放弃"1 SW 多 client"模型，明文声明"1 SW 1 client"；MultiTab 6-2~6-5 改 skip；调整架构文档
- **c''**：WebRTC 层重构（大改，不推荐短期）
