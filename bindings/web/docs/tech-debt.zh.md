...
**但 MultiTab 6-2/6-3 仍 FAIL**：同样是 "Echo not working within 60000ms"。说明 TD-004 并不是 6-2/6-3 的**唯一** blocker，存在第二层独立 bug（符合诊断 agent 的"次要发现"）。需要独立立项继续查（可能是 WebRTC 多 client 建联路径、WebSocket 信令多路复用、或 2-client-same-SW 场景下 sw-host 状态共享的另一处隐含单例）。

---

## TD-005：WebWorkloadAdapter 与 protoc-gen Workload 交互在 echo dispatch 时报 `Reflect.get called on non-object`

**发现时间**：2026-04-24
**发现路径**：Option U Phase 6c —— echo guest 一份化尝试
**相关分支**：`p6c`（未合 main；commits `4fdac3de` / `b63bafbc` / `48e01f95`）

### 现象

Phase 6c 把 echo 的 `server-guest` 改为用 `actr_framework::entry!(EchoServiceWorkload<EchoService>, ...)`（统一架构），删除 `server-guest-wbg` 副本。`cargo check` 对 `wasm32-wasip2` 和 `wasm32-unknown-unknown --features web --no-default-features` 两个 target 都 PASS。`wasm-pack build` + `start-mock-wbg.sh` 构建步骤也 PASS。

但跑 BasicFunction e2e：

```
📤 Sending: "echo-probe-..."
❌ Request failed: RPC error [500]: guest dispatch rejected: 
  JsValue(Error: [WBG] guest dispatch returned Err: 
    {"internal":"Reflect.get called on non-object"}
    at dispatchFn (http://localhost:5173/actor.sw.js:421:19))
```

100% 复现。在**非** unified 的 main（`1ac5afd4`，`*-guest-wbg` 还在）上，BasicFunction 6/6 PASS。

### 定位

报错说明：
- guest dispatch **被成功调用了**（不是 T18 那种永挂）
- guest 代码返回了 `ActrError::Internal("Reflect.get called on non-object")`
- 某处 `js_sys::Reflect::get(value, key)` 被调用但 `value` 不是 JS object

对比两条链路：

- **老 `*-guest-wbg`**（工作）：直接 `impl actr_web_abi::host::Workload`，手写 17 方法；dispatch 直接拿 `envelope.payload` 当 bytes。
- **新统一 `*-guest`**（破）：`actr_framework::entry!` 展开 → `WebWorkloadAdapter<EchoServiceWorkload<EchoService>>` 实现 `actr_web_abi::host::Workload` → 再走 `<EchoServiceDispatcher as MessageDispatcher>::dispatch`（protoc-gen 生成）。

中间**新增了两层**：
1. `WebWorkloadAdapter::dispatch`（Phase 6b 新增）—— 把 `wit::RpcEnvelope` 转成 `actr_protocol::RpcEnvelope`
2. `EchoServiceDispatcher::dispatch`（protoc-gen）—— 按 route_key 分派 + 反序列化 EchoRequest

其中之一（大概率是 WebWorkloadAdapter 的 envelope 转换，因为它用 `serde_wasm_bindgen::from_value` + `Reflect::get` 来解析 JS 对象）在某个字段上把 JS 值当对象访问，实际是 primitive（bytes payload / null / undefined）。

### 间接证据

`framework-web-entry-smoke` crate（Phase 6b 留下的最小 compile test，用 **手写**简单 Workload 不经 protoc-gen）`cargo check` 通过 —— 但那是 compile-time 证据，不是 runtime。未验证 smoke crate e2e 表现。**推测**：单层 adapter 可能 OK，protoc-gen 生成的 dispatcher 格式和 adapter 期望不匹配。

### 暂缓理由

- Phase 6c 不合并不损失任何现有功能（`*-guest-wbg` 保留继续工作）
- Phase 6a-I + 6b 的架构成果已在 main（γ 并发隔离 + entry! 宏 + adapter + ServiceHandler 关联类型）
- 这是一个**定位清晰**的小 bug（而非架构问题），适合独立诊断 + 单 spike 修复

### 诊断入口

- `/tmp/actr-p6c/` worktree / `p6c` 分支保留着 3 个 commit，可直接 checkout 复现
- 关键嫌疑文件：`core/framework/src/web/adapter.rs`（`WebWorkloadAdapter::dispatch`）
- 对比参考：老 `server-guest-wbg/src/lib.rs` 的 `dispatch` 实现（怎么拿 envelope.payload）
- Runtime trace：在 adapter 的 `from_value` / `Reflect::get` 调用前后加 `console.log(envelope, typeof envelope)` 观察输入形状
- `serde_wasm_bindgen::from_value` 可能因为 enum tag 或 Option 的序列化形状不匹配而返回错误 —— 检查 wit::RpcEnvelope 的 derive(Deserialize) 和 actor-wbg.sw.js 实际传的 JS 对象键名

### 修复选项（占位）

- **a'**：修 `WebWorkloadAdapter::dispatch` 的 envelope 解析，使之和 actor-wbg.sw.js 的 dispatchFn 输入对齐
- **b'**：不用 protoc-gen 生成的 MessageDispatcher 走 framework dispatch 链路；让 `entry!(SomeHandlerType)` 直接展开成"手写 Workload 方法映射"，跳过 adapter
- **c'**：给 `framework-web-entry-smoke` 也加 e2e 测试，先确认 adapter 在**不经 protoc-gen** 时是否 work；再缩小怀疑面
