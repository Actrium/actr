...
pub trait IStreamTestService: actor_rtc_framework::local_actor::LocalActor {
    type ServerStreamStream: futures_util::Stream<Item = actor_rtc_framework::error::ActorResult<TestResponse>>
        + Send
        + 'static;
    async fn unary_call(
        &self,
    ...
    ) -> actor_rtc_framework::error::ActorResult<TestResponse>;
    ...
    async fn server_stream(
        ...
        request: TestRequest,
        context: std::sync::Arc<actor_rtc_framework::context::Context>,
    ) -> actor_rtc_framework::error::ActorResult<Self::ServerStreamStream>;
    async fn bi_directional(
        &self,
        _request: TestRequest,
        _context: std::sync::Arc<actor_rtc_framework::context::Context>,
    ) -> actor_rtc_framework::error::ActorResult<TestResponse> {
...
#[allow(dead_code)]
pub struct StreamTestServiceClient {
    remote_actor: actor_rtc_framework::remote_actor::RemoteActor,
}
#[allow(dead_code)]
impl StreamTestServiceClient {
...
pub struct StreamTestServiceAdapter;
impl actor_rtc_framework::routing::RouteProvider<dyn IStreamTestService>
    for StreamTestServiceAdapter
{
    fn get_routes(
        actor: std::sync::Arc<dyn IStreamTestService>,
    ) -> Vec<actor_rtc_framework::routing::Route> {
        vec![
            actor_rtc_framework::routing::Route {
                method_name: "stream.test.StreamTestService/UnaryCall".to_string(),
                handler: {
                    ...
                    Box::new(
                        move |ctx: std::sync::Arc<actor_rtc_framework::context::Context>,
                            ...
                            Box::pin(async move {
                                use prost::Message;
                                let request = TestRequest::decode(&*req_bytes).map_err(|e| {
                                    actor_rtc_framework::error::ActorError::Protocol(format!(
                                        "Failed to decode {}: {}",
                                        "stream_test.TestRequest", e
                                    ))
                                })?;
                                let response: TestResponse =
                                    actor_for_task.unary_call(request, ctx).await.map_err(|e| {
            ...
            actor_rtc_framework::routing::Route {
                method_name: "stream.test.StreamTestService/ClientStream".to_string(),
                handler: {
                    let actor_clone = actor.clone();
                    Box::new(
                        move |ctx: std::sync::Arc<actor_rtc_framework::context::Context>,
                            ...
                            Box::pin(async move {
                                use prost::Message;
                                let message = TestRequest::decode(&*req_bytes).map_err(|e| {
                                    actor_rtc_framework::error::ActorError::Protocol(format!(
                                        "Failed to decode {}: {}",
                                        "stream_test.TestRequest", e
                                    ))
            ...
            actor_rtc_framework::routing::Route {
                method_name: "stream.test.StreamTestService/ServerStream".to_string(),
                handler: {
                    let actor_clone = actor.clone();
                    Box::new(
                        move |ctx: std::sync::Arc<actor_rtc_framework::context::Context>,
            ...
            actor_rtc_framework::routing::Route {
                method_name: "stream.test.StreamTestService/BiDirectional".to_string(),
                handler: {
                    let actor_clone = actor.clone();
                    Box::new(
                        move |ctx: std::sync::Arc<actor_rtc_framework::context::Context>,
...
impl<T> actor_rtc_framework::routing::RouteProvider<T> for StreamTestServiceAdapter
where
    T: IStreamTestService + Send + Sync + 'static,
{
    fn get_routes(actor: std::sync::Arc<T>) -> Vec<actor_rtc_framework::routing::Route> {
        let trait_obj: std::sync::Arc<dyn IStreamTestService> = actor;
        Self::get_routes(trait_obj)
    }
}
/// Auto-generated RemoteActor client manager for #manager_ident
#[allow(dead_code)]
pub struct StreamTestServiceClientManager {
    manager:
        std::sync::Arc<tokio::sync::RwLock<actor_rtc_framework::remote_actor::RemoteActorManager>>,
}
#[allow(dead_code)]
impl StreamTestServiceClientManager {
    ...
    pub async fn register_remote_actor(
        ...
        manager.register_remote_actor(
            actor_id,
            stringify!(StreamTestServiceClientManager).to_string(),
            service_address,
        )
    }
    pub async fn unary_call(
