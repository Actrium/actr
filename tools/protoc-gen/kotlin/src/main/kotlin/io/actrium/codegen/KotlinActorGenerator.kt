...
/** Declaring owner of a proto message type: package + proto file name. */
data class TypeOwner(val packageName: String, val protoFileName: String)
...
class KotlinActorGenerator(
    ...
    private fun generateImports(): String {
        // Collect every declaring proto file referenced by the service's RPC
        // methods (resolved through the owner map built from the full
        // descriptor set) so imported types resolve to their real owner outer
        // class instead of the current service's package.
        val owners = methods
            .flatMap { listOf(it.inputType, it.outputType) }
            .map { resolveOwner(it) }
            .toMutableSet()
        // Always include the current service's own file.
        owners.add(TypeOwner(packageName, protoFileName))

        val importLines = owners
            .map { importPathFor(it) }
    ...
    /** Kotlin import path (`package.OuterClass`) for a declaring owner. */
    private fun importPathFor(owner: TypeOwner): String {
        val outer = outerClassFor(owner.protoFileName)
        return if (owner.packageName.isEmpty()) outer else "${owner.packageName}.$outer"
    }
