...
impl AdminConfig {
    ...
    pub fn validate(&self) -> Result<()> {
        if self.node_id.is_empty() {
            return Err(AdminError::Config("node_id cannot be empty".to_string()));
        }

        if self.endpoint.is_empty() {
            return Err(AdminError::Config("endpoint cannot be empty".to_string()));
        }

        if self.agent_addr.is_empty() {
            return Err(AdminError::Config("agent_addr cannot be empty".to_string()));
        }
