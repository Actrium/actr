...
use chrono::Utc;
use platform::realm::{Realm, RealmConfig};
...
pub async fn load_realm_metadata(realm_rowid: i64) -> Result<RealmMetadata, AdminError> {
...
async fn load_enabled_flag(realm_rowid: i64) -> Result<bool, AdminError> {
    let config = RealmConfig::get_by_realm_and_key(realm_rowid, REALM_ENABLED_KEY)
        .await
        .map_err(|e| AdminError::Internal(format!("Failed to load realm enabled flag: {e}")))?;
...
async fn load_version(realm_rowid: i64) -> Result<u64, AdminError> {
    let config = RealmConfig::get_by_realm_and_key(realm_rowid, REALM_VERSION_KEY)
        .await
        .map_err(|e| AdminError::Internal(format!("Failed to load realm version: {e}")))?;
...
async fn load_use_servers(realm_rowid: i64) -> Result<Vec<ResourceType>, AdminError> {
    let config = RealmConfig::get_by_realm_and_key(realm_rowid, REALM_USE_SERVERS_KEY)
        .await
        .map_err(|e| AdminError::Internal(format!("Failed to load realm services: {e}")))?;
...
async fn upsert_config_value(realm_rowid: i64, key: &str, value: String) -> Result<(), AdminError> {
    let existing = RealmConfig::get_by_realm_and_key(realm_rowid, key)
        .await
        .map_err(|e| AdminError::Internal(format!("Failed to read realm config: {e}")))?;
    ...
    match existing {
        Some(mut cfg) => {
            ...
            cfg.save()
                .await
                .map_err(|e| AdminError::Internal(format!("Failed to update realm config: {e}")))?;
        }
        None => {
            let mut cfg = RealmConfig::new(realm_rowid, key.to_string(), value);
            cfg.save()
                .await
                .map_err(|e| AdminError::Internal(format!("Failed to create realm config: {e}")))?;
        }
    }
...
fn serialize_use_servers(use_servers: &[ResourceType]) -> Result<String, AdminError> {
    let values: Vec<i32> = use_servers.iter().map(|v| *v as i32).collect();
    serde_json::to_string(&values)
        .map_err(|e| AdminError::Internal(format!("Failed to serialize use_servers: {e}")))
}
