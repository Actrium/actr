...
impl AdminClient {
    /// 创建新的 admin 客户端（使用服务收集器）
    pub fn new(config: AdminConfig, service_collector: ServiceCollector) -> Result<Self> {
        config.validate()?;

        // 解码共享密钥
        let shared_secret = if let Some(ref secret_hex) = config.shared_secret {
            hex::decode(secret_hex)
                .map_err(|e| AdminError::Config(format!("Invalid shared_secret hex: {e}")))?
        } else {
    ...
    pub async fn connect(&mut self) -> Result<()> {
        ...
        let mut endpoint = Endpoint::from_shared(self.config.endpoint.clone())
            .map_err(|e| AdminError::Config(format!("Invalid server address: {e}")))?
    ...
    fn build_tls_config(&self) -> Result<ClientTlsConfig> {
        ...
        let tls_domain = self.config.tls_domain.as_ref().ok_or_else(|| {
            AdminError::Config("tls_domain is required when enable_tls is true".to_string())
        })?;
...
mod tests {
    ...
    fn test_client_creation() {
        ...
        let client = AdminClient::new(config, service_collector);
        assert!(client.is_ok());
    }
    ...
    async fn test_build_service_advertisements_merges_tags() {
        ...
        let client = AdminClient::new(config, service_collector).unwrap();
    ...
    async fn test_create_report_request() {
        let secret =
            hex::decode("0123456789abcdef0123456789abcdef0123456789abcdef0123456789abcdef")
                .unwrap();
        let service_collector = ServiceCollector::new();
        let report = AdminClient::create_report_request(
            "test-node",
            "test-location",
            "test-name",
            &secret,
    ...
    fn test_tls_config_validation() {
        ...
        let service_collector = ServiceCollector::new();
        let result = AdminClient::new(config, service_collector);
