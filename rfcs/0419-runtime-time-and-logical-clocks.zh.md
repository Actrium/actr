...
1. 运行时使用进程内单调时钟计算 timeout、期限、退避和耗时；
...
实现提供平台内部的最小抽象。它不进入公开 API surface；宿主 crate 与跨运行时
...
native 运行时可以使用 `std::time::Instant`（在其挂起语义满足契约的平台上）
...
本地期限；不得把旧进程的 monotonic instant 恢复到新进程。重算的时基是必须
显式选择的取舍：
...
- `SignalingEnvelope.timestamp` 已弃用：仅存诊断用途，正按分段计划退役
  ...
  信封时的读数，全部 flow 的信封（含 envelope_error）同此；它不是服务端
  授时，读方不得将其用于因果排序、timeout、授权或任何其它判定。
...
- tracing 中的 send/receive 时间戳是对应节点的观测时间，不覆盖 payload 中的
  业务事件时间。
...
3. 统一 native、Web 和测试运行时的单调时钟抽象与虚拟时间测试；
   Web 侧现存以 `Date.now()` 差值实现的超时判断是主要工作量；
4. 增加独立、可选且不依赖 `core/protocol` 的 HLC 工具库。

验收标准：核心报文 diff 为空（proto 注释修正不改变 schema）；现有 RPC 和数据流
