...
//!
//! The production failure was not that signaling stopped. The failing contract is
//! narrower: after the answerer observes WebRTC `Disconnected`/`Failed`, new
//! outbound RPCs from that answerer must not continue to use the old cached
//! `DestTransport`/DataChannel just because `WirePool` still reports WebRTC as
//! ready or the local DataChannel state still looks open.
//!
//! The expected recovery driver is the offerer, but in the failing case the
//! offerer side may already have closed/removed the connection and will never
//! initiate an ICE restart. The answerer therefore must stop treating the stale
//! WebRTC lane as sendable and should fail fast, or wait until cleanup removes
//! the stale transport so a later send can build a fresh connection.

mod common;

use actr_protocol::{ActrId, PayloadType, RpcEnvelope};
use actr_runtime::transport::{ConnectionEvent, ConnectionState, Dest};
